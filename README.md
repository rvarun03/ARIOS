...
> **Project status:** ARIOS is an active local-development project. The core ingestion, LangGraph workflow, NLP, semantic search, and RAG paths are implemented; authentication, deployment, and production hardening are not.

## Project overview

ARIOS turns unstructured content into searchable document knowledge. It accepts web pages, YouTube transcripts, GitHub repositories, PDFs, and images, extracts their text, and stores document records in SQLite. A LangGraph ingestion workflow validates inputs, detects the source type, runs ingestion, performs NLP analysis, saves the document, and indexes it. The NLP pipeline adds statistics, named entities, keywords, an extractive summary, and a zero-shot document classification.
...
- LangGraph-based ingestion workflow with automatic source type detection
...
- Experimental hybrid retrieval using query expansion, keyword search, and rank-based result merging
...
- Transformer-based semantic reranking using a CrossEncoder to reorder retrieved chunks before RAG
- Retrieval evaluation for comparing semantic, semantic-expanded, hybrid, and reranked retrieval modes
- Source-aware text cleaning and section-aware chunking for web pages, PDFs, and YouTube transcripts
- Improved YouTube ingestion with transcript cleaning, timestamp/segment metadata, and video-level question answering
- Structured source citations in generated answers
- Docker-based backend deployment with persistent SQLite and ChromaDB volumes
- Cloud VM deployment for the FastAPI backend and frontend deployment for the client UI
...
```mermaid
flowchart LR
    A[Web / YouTube / GitHub / PDF / Image] --> B[FastAPI + LangGraph ingestion workflow]
    B --> C[Text extraction]
...
1. A client submits a source through `POST /documents/graph/ingest`, or uploads a PDF/image through `POST /documents/graph/upload`.
2. LangGraph validates the input, detects the source type, and routes the source to the matching ingestion adapter.
3. The matching ingestion adapter extracts text into a common schema.
4. The NLP service cleans and normalizes the text, creates a spaCy document, and extracts statistics, entities, keywords, and a summary.
5. BART-MNLI scores the text against eight candidate document labels.
6. ARIOS stores the document, text, and NLP metadata in SQLite.
7. The document is chunked, embedded, and indexed in ChromaDB.
8. Semantic search embeds a query and retrieves the closest chunks, optionally constrained by source type or document ID.
9. The `/documents/ask` route combines retrieved chunks with stored analysis metadata and sends a context-restricted prompt to Groq.

## LangGraph ingestion workflow

ARIOS uses LangGraph to organize ingestion as a clear multi-step workflow:
...
↓
Chunk, embed, and index in ChromaDB
```

This keeps ingestion, analysis, persistence, and indexing separated into clear workflow stages.
...
2. Retrieve relevant chunks from ChromaDB using semantic search by default.
...
The current route returns the generated answer as a JSON string. Semantic retrieval is the default path; experimental retrieval modes can be used while testing ranking quality. Although retrieved metadata is used internally, structured source citations are not yet included in the response.
...
| `POST` | `/documents/graph/ingest` | Runs LangGraph ingestion for a source with automatic source type detection |
| `POST` | `/documents/graph/upload` | Runs LangGraph ingestion for uploaded files |
| `POST` | `/documents/ingest` | Ingests, analyses, saves, chunks, embeds, and indexes a source |
| `POST` | `/documents/upload` | Uploads and analyses a PDF or image |
...
| `GET` | `/documents/vector-store/count` | Returns the ChromaDB collection count |
...
ARIOS/
├── back-end/
│   ├── agents/               # LangGraph ingestion workflow
...
```bash
curl -X POST "http://127.0.0.1:8000/documents/graph/ingest" \
...
Upload a PDF through the LangGraph upload route:

```bash
curl -X POST "http://127.0.0.1:8000/documents/graph/upload" \
...
- **LangGraph:** structures ingestion as a clear workflow with validation, source detection, ingestion, analysis, persistence, and indexing stages.
...
- **Experimental hybrid retrieval:** combines query expansion, keyword search, and rank-based merging for retrieval experiments, while semantic search remains the default.
